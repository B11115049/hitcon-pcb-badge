...
class IrInterface:
    ...
    async def get_next_packet(self) -> bytes:
        ...
        for _ in range(self.failure_try):
            try:     
                await asyncio.wait_for(self._read_until_response(self.read_packet_que), timeout=self.timeout)
    ...
    async def _write_packet_and_wait_for_response(self, packet: Packet) -> Packet:
        ...
        try:
            await asyncio.wait_for(self._read_until_response(self.waiting_que_table[waiting_que_key]), timeout=self.timeout)
        ...
        with self.serial as s:
            if lock:
                async with self._get_lock(read=False):
                    await asyncio.wait_for(asyncio.to_thread(self.serial.write, PC.PREAMBLE.value + data), timeout=None)
            else:
                await asyncio.wait_for(asyncio.to_thread(self.serial.write, PC.PREAMBLE.value + data), timeout=None)
